...
#Script for making input files for LuxUS HS. The idea is to make windows of fixed size and analyze the cytosines in that window at a time. One input file is produced per window.
...
if __name__ == '__main__':
    ...
    parser.add_argument('-x','--prepare_data_for_LuxUS_HS',action='store',dest='LuxUS_HS',type=int,required=True,help='Give value 0 or 1.')
    ...
    for i in range(0,len(BSseq_data)):
        ...
        if (((i==(len(BSseq_data)-1)) and CYTOSINE_INDEX_WINDOW>=1) or (((BSseq_data[i+1][1]-WINDOW_START_COORD)>window_width)) and CYTOSINE_INDEX_WINDOW>=1) or CYTOSINE_INDEX_WINDOW>=window_N_cytosines:
            ...
            if pvalue<=options.req_meth_diff and N_samples_with_required_coverage_group1>=N_required_samples and N_samples_with_required_coverage_group2>=N_required_samples:             
                ...
                if options.LuxUS_HS==1:
                    print("The data is saved in a format appropriate for LuxUS-HS")
                    luxushs_data={'n_cytosines': int(CYTOSINE_INDEX_WINDOW),'n_replicates': int(N_replicates),'n_predictors': int(N_predictors),
                    ...
                    pickle.dump(luxushs_data,output4)
                    output4.close()
                ...
                #print("The linear model coefficient for the chosen variable of interest was smaller than %s for window %s. Window is not saved."%(options.req_meth_diff,WINDOW_COUNT))
