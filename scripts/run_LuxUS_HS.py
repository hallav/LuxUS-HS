...
def run_luxus_HMC(sigmaB2, lux_data, stan_model_file, plot_file_name, covariate_to_be_tested, N_outputsamples, N_chains, diagnostic_plots,testtype,test2cov,N_cytosines,N_predictors):
    ...
    return bf, runtime, sigmaR2_mean, sigmaE2_mean, d_median[::N_predictors], theta_median, d_mean[::N_predictors], theta_mean;	
...
if __name__ == '__main__':
    ...
    parser.add_argument('-k','--fileIdentifier',action='store',dest='fileIdentifier',type=str,required=True,help='File identifier for the d and theta mean files.')	
    ...
    d_median_file="d_median_algorithm%s_%s.txt"%(algorithm,options.fileIdentifier)
    theta_median_file="theta_median_algorithm%s_%s.txt"%(algorithm,options.fileIdentifier)
    d_mean_file="d_mean_algorithm%s_%s.txt"%(algorithm,options.fileIdentifier)
    theta_mean_file="theta_mean_algorithm%s_%s.txt"%(algorithm,options.fileIdentifier)
    ...
    if algorithm==0:
        ...
        BF, runtime, sigmaR2_mean, sigmaE2_mean, d_median, theta_median, d_mean, theta_mean = run_luxus_HMC(sigmaB2, luxus_data, stan_file, plot_file_name, options.test_covariate, N_outputsamples_HMC,N_HMC_chains,diagnostic_plots,test_type,test_type2_cov,luxus_data['n_cytosines'],N_covariates)
    ...
    if len(d_median)>1:
        with open("%s/%s"%(options.outputFolder,d_median_file),'a+') as fw:
            for c_ind in range(0,luxus_data['n_cytosines']):
                fw.write("%f\t%s\t%s\n"%(d_median[c_ind],options.window_index,c_ind))
        with open("%s/%s"%(options.outputFolder,d_mean_file),'a+') as fw:
            for c_ind in range(0,luxus_data['n_cytosines']):
                fw.write("%f\t%s\t%s\n"%(d_mean[c_ind],options.window_index,c_ind))
    ...
    with open("%s/%s"%(options.outputFolder,theta_mean_file),'a+') as fw:
        for t_ind in range(0,N_replicates*luxus_data['n_cytosines']):
            fw.write("%f\t%s\t%s\n"%(theta_median[t_ind],options.window_index,t_ind))   
