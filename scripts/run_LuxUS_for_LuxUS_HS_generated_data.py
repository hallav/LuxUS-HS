...
def run_luxus_HMC(sigmaB2, lux_data, stan_model_file, plot_file_name, covariate_to_be_tested, N_outputsamples, N_chains, diagnostic_plots,testtype,test2cov,N_cytosines):
    ...
    if diagnostic_plots==1:
        if N_cytosines ==1:
            ...
            vars_to_be_printed=['l','sigmaE2','sigmaR2','sigmaC2','B']
...
if __name__ == '__main__':
    ...
    N_cytosines=luxus_data['n_cytosines']
    
    #Change the dict object to match to the LuxUS Stan code
    D = generate_D(N_covariates,N_replicates)
    luxus_data['X'] = numpy.tile(D,(N_cytosines,1))
    luxus_data['coordinates']=luxus_data['coordinates'][::N_covariates]
    luxus_data['alphaC']=6 #Using the same values as in LuxUS scripts.
    luxus_data['betaC']=3
    luxus_data['Z_C']=numpy.kron(numpy.arange(1,luxus_data['n_cytosines']+1),numpy.ones(N_replicates)).astype(int)
    ...
    theta_median_file="LuxUS_theta_median_algorithm%s_%s.txt"%(algorithm,input_data_id)
    ...
    if algorithm==0:
        ...
        plot_file_name="%s/LuxUS_%s_diagnostic_plots_HMC.png"%(options.outputFolder,input_data_id)
        ...
        BF, runtime, sigmaR2_mean, sigmaE2_mean, theta_median = run_luxus_HMC(sigmaB2, luxus_data, stan_file, plot_file_name, options.test_covariate, N_outputsamples_HMC,N_HMC_chains,diagnostic_plots,test_type,test_type2_cov,luxus_data['n_cytosines'])
    ...
    if options.algorithm==1:
        ...
        plot_file_name="%s/LuxUS_%s_diagnostic_plots_ADVI.png"%(options.outputFolder,input_data_id)


        print("Estimating the model parameters with VI.")

        variational_temp_data_file="%s/TEMP_LuxUS_store_variational_input_%s_%s_%s_%s_%s_%s_%s.txt"%(options.outputFolder,input_data_id,currenttime[0],currenttime[1],currenttime[2],currenttime[3],currenttime[4],currenttime[5])
        variational_temp_output_file="%s/TEMP_LuxUS_store_variational_results_%s_%s_%s_%s_%s_%s_%s.csv"%(options.outputFolder,input_data_id,currenttime[0],currenttime[1],currenttime[2],currenttime[3],currenttime[4],currenttime[5])

        BF, runtime, sigmaR2_mean, sigmaE2_mean, theta_median = run_luxus_VI(luxus_data,stan_file, N_gradsamples, N_elbosamples, N_outputsamples_VI, variational_temp_data_file, variational_temp_output_file, sigmaB2,diagnostic_plots,plot_file_name,N_covariates, N_replicates, options.test_covariate,test_type,test_type2_cov,luxus_data['n_cytosines'])
